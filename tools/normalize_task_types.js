...
const REMOVED_RAW_TASK_TYPE_KEY = ["task", "type", "raw"].join("_");
...
function normalizeFile(filePath) {
  ...
  for (const item of items) {
    ...
    if (!CANONICAL_TASK_TYPES.has(normalized)) {
      throw new Error(`Unmapped task_type "${item.task_type}" in ${filePath} (${item.id || "unknown id"})`);
    }

    item.task_type = normalized;
    delete item[REMOVED_RAW_TASK_TYPE_KEY];
...
function orderedItem(item) {
  ...
  for (const key of Object.keys(item)) {
    if (key !== REMOVED_RAW_TASK_TYPE_KEY && !Object.prototype.hasOwnProperty.call(out, key)) {
      out[key] = item[key];
    }
  }
  return out;
