...
- **[2026-08]** SportsTime has been accepted to **ECCV 2026**.
...
- **[2026-04]** Video access is handled through the gated [Hugging Face dataset](#videos).

---

**SportsTime** is a large-scale benchmark for temporal compositional reasoning in long-form sports videos, accepted to **ECCV 2026**.
...
We provide the official video-level train/test split in [`splits/split.json`](splits/split.json). No `video_id` appears in both train and test.

| Split | Samples | Video IDs |
|---|---:|---:|
| Train | 11,540 | 1,546 |
| Test | 2,786 | 391 |

Use the `train.sample_ids` and `test.sample_ids` fields for sample-level filtering. The split applies to both `data/` and `data_en/`.
...
The video files are hosted in the gated [SportsTime Hugging Face dataset](https://huggingface.co/datasets/Ustiniansy/SportsTime) under `videos/`.

Access requests are reviewed manually, usually within one week. Please use the videos only for non-commercial research or educational purposes, and do not redistribute the video files or private access links.

## Citation

SportsTime has been accepted to **ECCV 2026**. If you use SportsTime in your research, please cite the arXiv version below; the proceedings citation will be updated when available.
